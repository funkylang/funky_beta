...
def update_footer(filepath, content_changed=False):
    """Update the 'generated by' and timestamp footer lines.

    Returns True if the file was modified. The timestamp and credit are only
    refreshed when the page is actually changing (footer lines added/removed,
    or the caller reports a content change). Re-running on an unchanged page
    leaves it byte-identical — no timestamp bump, no credit rewrite."""
    p = Path(filepath)
    ...
    # A ((revised)) line below will trigger a change — count it up front so the
    # timestamp/credit refresh decision is known before we reach those lines.
    if any(l.strip() == "((revised))" for l in lines):
        changed = True

    for i, line in enumerate(lines):
        stripped = line.strip()
        # Update timestamp — only when the page is actually changing, so a
        # re-run on an unchanged page leaves the file byte-identical
        if re.match(r"^\(\(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\)\)$", stripped):
            if changed:
                indent = line[:len(line) - len(line.lstrip())]
                lines[i] = indent + now
        elif re.search(r"\(\(\d{4}-\d{2}-\d{2}", stripped):
            # Malformed timestamp — replace anyway (a real fix)
            indent = line[:len(line) - len(line.lstrip())]
            lines[i] = indent + now
            changed = True
        # Update generated by / credit — only when the page is actually changing
    ...
    # Drop the removed ((revised)) markers now so every later pass over `lines`
    # (credit/timestamp checks, insert-position scan) sees real strings only.
...
def process_doc(path):
    ...
    if not example_starts:
        ...
        update_footer(doc_path)  # still add 'defined in' etc. if missing
        return False
    ...
    doc_path.write_text('\n'.join(lines))
    update_footer(doc_path, content_changed=content_changed)
    if content_changed:
