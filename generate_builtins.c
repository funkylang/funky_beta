// generate_builtins.c
//
// Reimplementation (in C) of the Simplicity program
// "old_compiler/builtins.sim". It reads the runtime templates
// ("runtime_templates/*.template") plus the runtime VTABLE definition
// ("runtime/common.h") and regenerates the two files
//   runtime/builtins.h
//   runtime/builtins.c
...
//   in builtins.sim) and emits them in that table's iteration order, which is
//   an implementation-defined artifact of the Simplicity/Funky hash-table (a
//   HAMT). This tool instead emits them in a stable, deterministic order (the
//   order in which the UNION directives appear in the templates).  These 13
//   typedefs are mutually independent (each is "{ TAG tag; long offset; <type>
//   value; }"), so the reordering is semantically irrelevant: the header
//   compiles identically.
//
// Usage:
//   generate_builtins [templates_dir] [common.h] [out.h] [out.c]
//   defaults: runtime_templates  runtime/common.h  runtime/builtins.h
//   runtime/builtins.c
...
static String *string_of_bytes(const char *bytes, long length) {
  ...
  if (length) {
    memcpy(s->data, bytes, (size_t)length);
  }
  s->data[length] = '\0';
  s->length = length;
  return s;
...
static String *empty_string(void) {
  return string_of_bytes("", 0);
}

static int is_empty(const String *s) {
  return s->length == 0;
}
...
static int needle_match_at(const String *s, const Needle *nd, long p) {
  ...
  case NK_STRING:
    if (p < 1 || p + nd->slen - 1 > s->length) {
      return 0;
    }
    return memcmp(s->data + (p - 1), nd->str, (size_t)nd->slen) == 0
             ? (int)nd->slen
             : 0;
  }
  return 0;
...
// Returns 1-based start position (0 when not found) and stores the match
// length.
...
static String *between_needles(const String *s, Needle a, Needle b, int occ) {
  int la;
  long pa = find_needle(s, a, occ, &la);
  if (!pa) {
    return empty_string();
  }
...
static int contains_c(const String *s, const char *sub) {
  int L;
  return find_needle(s, need_string(sub), 1, &L) != 0;
}
static int contains_char(const String *s, int c) {
  int L;
  return find_needle(s, need_char(c), 1, &L) != 0;
}
...
static String *trim_right(const String *s) {
  long j = s->length;
  while (j >= 1 && is_ws((unsigned char)s->data[j - 1])) {
    j--;
  }
...
static String *trim(const String *s) {
  long i = 1, j = s->length;
  while (i <= j && is_ws((unsigned char)s->data[i - 1])) {
    i++;
  }
  while (j >= i && is_ws((unsigned char)s->data[j - 1])) {
    j--;
  }
  if (i > j) {
    return empty_string();
...
static String *without_suffix_c(const String *s, const char *suffix) {
  if (has_suffix(s, suffix)) {
    return range_of(s, 1, s->length - (long)strlen(suffix));
  }
  return string_of_bytes(s->data, s->length);
}
...
static String *before_char(const String *s, int c) {
  return before_needle(s, need_char(c), 1);
}
static String *behind_char(const String *s, int c) {
  return behind_needle(s, need_char(c), 1);
}
static String *truncate_from_char(const String *s, int c) {
  return truncate_from_needle(s, need_char(c), 1);
}
static String *truncate_until_str(const String *s, const char *x) {
  return truncate_until_needle(s, need_string(x), 1);
}
static String *before_str(const String *s, const char *x) {
  return before_needle(s, need_string(x), 1);
}
static String *behind_str(const String *s, const char *x) {
  return behind_needle(s, need_string(x), 1);
}
static String *truncate_from_str(const String *s, const char *x) {
  return truncate_from_needle(s, need_string(x), 1);
}
...
static void search_character(const String *text, int ch, long *idx) {
  while (*idx <= text->length && char_at(text, *idx) != ch) {
    (*idx)++;
  }
}
  ...
  if (char_at(text, idx) == '#' &&
    ...
    for (;;) {
      ...
      if (next_indent <= indent) {
        return range_of(text, line_start_idx, -1);
      }
    }
  }
  return string_of_bytes(text->data, text->length);
...
static StringList *slist_new(void) {
  ...
  l->cap = 8;
  l->count = 0;
  l->items = xmalloc(sizeof(String *) * l->cap);
  return l;
}
static void slist_push(StringList *l, String *s) {
  if (l->count == l->cap) {
    l->cap *= 2;
    l->items = realloc(l->items, sizeof(String *) * l->cap);
  }
  l->items[l->count++] = s;
}
...
static long rule_match(const String *s, long i, const RRule *r) {
  ...
  for (int e = 0; e < r->nelems; e++) {
    if (r->elems[e].is_ws) {
      long q = p;
      while (q < s->length && is_ws((unsigned char)s->data[q])) {
        q++;
      }
      if (q == p) {
        return -1; // need at least one whitespace char
      }
      p = q;
    } else {
      ...
      if (memcmp(s->data + p, r->elems[e].lit, (size_t)n) != 0) {
        return -1;
      }
      p += n;
    }
...
static String *replace_all(const String *s, const RRule *rules, int nrules) {
  ...
  while (i < s->length) {
    ...
    for (int r = 0; r < nrules; r++) {
      ...
      if (m >= 0) {
        builder_append_c(b, rules[r].repl);
        i += m;
        matched = 1;
        break;
      }
    ...
    if (!matched) {
      builder_append_char(b, (unsigned char)s->data[i]);
      i++;
    }
  }
  return builder_to_string(b);
...
static String *replace_first(
  const String *s, const char *from, const char *to) {
  int L;
  long p = find_needle(s, need_string(from), 1, &L);
  if (!p) {
    return string_of_bytes(s->data, s->length);
  }
  Builder *b = builder_new();
...
static String *type_and_name(const String *type, const String *name) {
  if (char_at(type, -1) == '*') {
    return concat2(type, name);
  }
  return concat2(concat_c(type, " "), name);
}

// extract_declaration(code): keep up to the first ')', then normalise
// whitespace.
...
static PairList *plist_new(void) {
  ...
  l->cap = 4;
  l->count = 0;
  l->items = xmalloc(sizeof(Pair) * l->cap);
  return l;
}
static void plist_push(PairList *l, String *k, String *v) {
  if (l->count == l->cap) {
    l->cap *= 2;
    l->items = realloc(l->items, sizeof(Pair) * l->cap);
  }
  l->items[l->count].key = k;
  l->items[l->count].value = v;
  l->count++;
}
...
static ConstList *clist_new(void) {
  ...
  l->cap = 16;
  l->count = 0;
  l->items = xmalloc(sizeof(Constant *) * l->cap);
  return l;
}
...
static void clist_push(ConstList *l, Constant *c) {
  if (l->count == l->cap) {
    l->cap *= 2;
    l->items = realloc(l->items, sizeof(Constant *) * l->cap);
  }
  l->items[l->count++] = c;
}
...
static VarList *vlist_new(void) {
  ...
  l->cap = 16;
  l->count = 0;
  l->items = xmalloc(sizeof(Variable *) * l->cap);
  return l;
}
static Variable *vlist_find(VarList *l, const String *name) {
  for (int i = 0; i < l->count; i++) {
    if (l->items[i]->fullname->length == name->length &&
        memcmp(l->items[i]->fullname->data, name->data, (size_t)name->length) ==
          0) {
      return l->items[i];
    }
  }
...
static void vlist_push(VarList *l, Variable *v) {
  if (l->count == l->cap) {
    l->cap *= 2;
    l->items = realloc(l->items, sizeof(Variable *) * l->cap);
  }
  l->items[l->count++] = v;
}
...
typedef struct {
  String *name;
  PairList *variants;
} Union; // variants: variant_name -> fields(PairList*) via value pointer hack
// We store union variants as: name -> list of (variant_name, PairList* of
// fields).
typedef struct {
  String *variant_name;
  PairList *fields;
...
static void varrec_reset(VarRecords *v) {
  v->count = 0;
  if (!v->items) {
    v->cap = 8;
    v->items = xmalloc(sizeof(StringList *) * v->cap);
  }
}
static void varrec_push(VarRecords *v, StringList *rec) {
  if (v->count == v->cap) {
    v->cap *= 2;
    v->items = realloc(v->items, sizeof(StringList *) * v->cap);
  }
  v->items[v->count++] = rec;
}
...
//   ends with '@'); a trailing '@' suppresses the following newline; '@'
//   renders as empty. "::" is left intact here and mangled by the recursive
//   pass.
static const StringMacro string_macros[] = {
  ...
  {"CREATE_RUNTIME_ERROR", "create_error_message(\n  "
                           "module__builtin.constants_base[unique__std__"
                           "RUNTIME_ERROR-1],\n  $1, 0, 0, NULL);"},
  {"RUNTIME_ERROR", "{\n  create_error_message(\n    "
                    "module__builtin.constants_base[unique__std__RUNTIME_ERROR-"
                    "1],\n    $1, 0, 0, NULL);\n  return;\n}"},
  {"NO_SUCH_FUNCTION", "{\n  no_such_function();\n  return;\n}"},
  {"NO_SUCH_ATTRIBUTE", "{\n  no_such_function();\n  return;\n}"},
  {"TOO_FEW_ARGUMENTS", "{\n  too_few_arguments();\n  return;\n}"},
...
static const int n_string_macros =
  (int)(sizeof(string_macros) / sizeof(string_macros[0]));
...
static int is_function_macro(const String *name) {
  static const char *names[] = {"EVENT", "ACTION", "INFO", "DATA", "INT",
    "UINT", "LONG", "ULONG", "PTR", "INT_ARRAY", "MEMORY", "C_STRING", "END",
    NULL};
...
static String *arg_or(StringList *args, int idx1, const char *dflt) {
  if (args && idx1 >= 1 && idx1 <= args->count) {
    return args->items[idx1 - 1];
  }
  return string_from_c(dflt);
}

// Build the DATA-phase "store" text for an integer-like variable.
static String *event_store_block(
  const char *store_fn, String *name, String *value, const char *end_call) {
  Builder *b = builder_new();
  if (g_event_kind == EK_EVENT) {
    ...
    builder_append_str(b, name);
    builder_append_c(b, ");\n");
    if (end_call && end_call[0]) {
      builder_append_c(b, "    ");
      builder_append_c(b, end_call);
      builder_append_c(b, "\n");
    }
    builder_append_c(b, "  }");
  }
...
#define NAME_IS(s) ((long)strlen(s) == nl && memcmp(s, n, (size_t)nl) == 0)
  ...
  if (NAME_IS("INT") || NAME_IS("UINT") || NAME_IS("LONG")) {
    ...
    String *value =
      (args && args->count == 2) ? args->items[1] : string_from_c("0");
    const char *rec_type = NAME_IS("INT")    ? "integer"
                           : NAME_IS("UINT") ? "unsigned_integer"
                                             : "long_integer";
    const char *store_fn = NAME_IS("INT")    ? "store__integer"
                           : NAME_IS("UINT") ? "store__unsigned_integer"
                                             : "store__long_integer";
    const char *end_call = NAME_IS("INT")    ? "end__record();"
                           : NAME_IS("LONG") ? "end__report();"
                                             : "";
  ...
  if (NAME_IS("ULONG")) {
    ...
    if (g_event_phase == EP_DATA) {
      varrec_push(
        &g_data_records, record3("unsigned_long_integer", name_a, value));
      return event_store_block(
        "store__unsigned_long_integer", name_a, value, "");
    }
    varrec_push(
      &g_info_records, record3("unsigned_long_integer", name_a, value));
    return empty_string();
  }
  if (NAME_IS("PTR")) {
    String *name_a = arg_or(args, 1, "");
    if (g_event_phase == EP_DATA) {
      ...
      if (g_event_kind == EK_EVENT) {
        Builder *b = builder_new();
        builder_append_c(b, "  store__pointer(");
  ...
  if (NAME_IS("INT_ARRAY") || NAME_IS("MEMORY")) {
    ...
    const char *sf =
      NAME_IS("INT_ARRAY") ? "store__int_array" : "store__memory";
    if (g_event_phase == EP_DATA) {
      varrec_push(&g_data_records, record3(rt, name_a, length));
      Builder *b = builder_new();
      builder_append_c(b, "  ");
      builder_append_c(b, sf);
      builder_append_c(b, "(");
      builder_append_str(b, name_a);
      builder_append_c(b, ", ");
      builder_append_str(b, length);
      builder_append_c(b, ");");
      return builder_to_string(b);
    }
  ...
  if (NAME_IS("C_STRING")) {
    ...
    if (g_event_phase == EP_DATA) {
      ...
      Builder *b = builder_new();
      builder_append_c(b, "  store__c_string(");
      builder_append_str(b, name_a);
      builder_append_c(b, ");");
      return builder_to_string(b);
    }
  ...
  if (NAME_IS("END")) {
    ...
    for (int i = 0; i < g_data_records.count; i++) {
      ...
      if (strcmp(type, "pointer") == 0) {
        builder_append_c(retr, "retrieve__pointer((const void **)&");
        builder_append_str(retr, vname);
        builder_append_c(retr, ");\n");
      } else if (strcmp(type, "int_array") == 0) {
        builder_append_str(retr, var->items[2]);
        builder_append_c(retr, " = retrieve__int_array((int **)&");
        builder_append_str(retr, vname);
        builder_append_c(retr, ");\n");
      } else if (strcmp(type, "memory") == 0) {
        String *v3 = var->items[2];
        if (has_prefix(v3, "sizeof(")) {
          builder_append_c(retr, "retrieve__fixed_memory((uint8_t *)");
          builder_append_str(retr, vname);
          builder_append_c(retr, ", ");
          builder_append_str(retr, v3);
          builder_append_c(retr, ");\n");
        } else {
          builder_append_str(retr, v3);
          builder_append_c(retr, " = retrieve__memory((uint8_t **)&");
          builder_append_str(retr, vname);
          builder_append_c(retr, ");\n");
        }
      } else {
        builder_append_c(retr, "retrieve__");
        builder_append_c(retr, type);
        builder_append_c(retr, "(&");
        builder_append_str(retr, vname);
        builder_append_c(retr, ");\n");
      }
    ...
    for (int i = 0; i < g_info_records.count; i++) {
      ...
      if (strcmp(type, "pointer") == 0) {
        builder_append_c(prnt, "print__pointer(");
        builder_append_str(prnt, vname);
        builder_append_c(prnt, ");\n");
      } else if (strcmp(type, "int_array") == 0) {
        builder_append_c(prnt, "print__int_array(");
        builder_append_str(prnt, vname);
        builder_append_c(prnt, ", ");
        builder_append_str(prnt, var->items[2]);
        builder_append_c(prnt, ");\n");
      } else if (strcmp(type, "memory") == 0) {
        builder_append_c(prnt, "print__memory(");
        builder_append_str(prnt, vname);
        builder_append_c(prnt, ", ");
        builder_append_str(prnt, var->items[2]);
        builder_append_c(prnt, ");\n");
      } else {
        builder_append_c(prnt, "print__");
        builder_append_c(prnt, type);
        builder_append_c(prnt, "(");
        builder_append_str(prnt, vname);
        builder_append_c(prnt, ");\n");
      }
    ...
    if (g_event_kind == EK_EVENT) {
      ...
      const char *type = var->items[0]->data;
      String *vname = var->items[1];
      String *dflt = (strcmp(type, "pointer") == 0)
                       ? string_from_c("SAMPLE_POINTER")
                       : var->items[2];
      builder_append_c(buf, "    ");
    ...
    if (char_at(rep, j) == '$') {
      ...
      if (var_no == '*') {
        for (int k = 0; k < args->count; k++) {
          if (k) {
            builder_append_c(out, ", ");
          }
          builder_append_str(out, args->items[k]);
        }
      } else if (var_no == '+') {
        ...
        if (idx >= 1 && idx <= args->count) {
          builder_append_str(out, args->items[idx - 1]);
        }
      }
      r = j + 1;
    }
      ...
      if ((chr >= 'a' && chr <= 'z') || (chr >= 'A' && chr <= 'Z') ||
        ...
        if (s) {
          if (chr == ':' && char_at(code, i + 1) == ':') {
            ...
            if (str_rep || is_fn) {
              ...
              if (chr == '(') {
                ...
                while (i <= len) {
                  ...
                  if (c == ',') {
                    if (level == 1) {
                      slist_push(arguments, trim(range_of(code, a, i - 1)));
                      a = i + 1;
                    }
                  } else if (c == '(') {
                    level++;
                ...
                if (str_rep) {
                  replacement = substitute_arguments(str_rep, arguments);
                } else {
                  replacement = call_function_macro(identifier, arguments);
                }
              } else {
                if (str_rep) {
                  replacement = string_from_c(str_rep);
                } else {
                  replacement = call_function_macro(identifier, slist_new());
                }
              }
              if (is_empty(replacement)) {
                ...
                while (
                  new_code->length > 0 &&
                  is_ws((unsigned char)new_code->data[new_code->length - 1])) {
                  new_code->length--;
                }
                new_code->data[new_code->length] = '\0';
              } else {
                String *line_start =
                  concat2(string_from_c("\n"), indentation(indent));
                ...
                for (long k = 0; k < expanded->length; k++) {
                  if (expanded->data[k] == '\n') {
                    ...
                    builder_append_char(
                      new_code, (unsigned char)expanded->data[k]);
                  }
                }
              }
              have_new_code = 1;
            ...
            // Note: `s` is reset ONLY here (in the non-"::" branch); a "::"
            // keeps the current identifier open so that qualified names
            // accumulate.
            s = 0;
          }
        }
...
static String *var_name(const String *name) {
  return concat2(string_from_c("var_"), (String *)name);
}
...
static void define_namespace(Tables *t, const char *ns) {
  for (int i = 0; i < t->defined_namespaces->count; i++) {
    if (strcmp(t->defined_namespaces->items[i]->data, ns) == 0) {
      return;
    }
  }
...
static void add_include_file(Tables *t, const String *filename) {
  for (int i = 0; i < t->include_files->count; i++) {
    if (strcmp(t->include_files->items[i]->data, filename->data) == 0) {
      return;
    }
  }
  ...
  t->include_files->items[pos] =
    string_of_bytes(filename->data, filename->length);
}

static Variable *get_or_define_variable(
  Tables *t, String *name, FieldObjectType proto_type) {
...
static void add_variable(Tables *t, String *name, Variable *newv) {
  ...
  if (nt == FOT_POLYMORPHIC) {
    if (ct == FOT_UNKNOWN || ct == FOT_UNKNOWN_POLYMORPHIC) {
      do_update = 1;
    } else {
      fprintf(stderr, "error: redefine %s\n", name->data);
      exit(1);
    }
  } else {
    if (ct == FOT_UNKNOWN) {
      do_update = 1;
    } else if (ct == FOT_UNKNOWN_POLYMORPHIC) {
      fprintf(stderr, "error: %s not polymorphic\n", name->data);
      exit(1);
    } else {
      fprintf(stderr, "error: redefine %s\n", name->data);
      exit(1);
...
static String *get_id(
  Tables *t, void *value, int is_string, String *sval, Constant *cval) {
  (void)value;
  if (is_string) {
    return sval;
  }
...
static void define_declaration(Tables *t, String *decl) {
  slist_push(t->declarations, decl);
}
static void define_local_function_declaration(Tables *t, String *d) {
  slist_push(t->local_function_declarations, d);
}
static void define_function_declaration(Tables *t, String *d) {
  slist_push(t->function_declarations, d);
}
static void define_code(Tables *t, String *code) {
  slist_push(t->code_sections, normalize_code(code));
}

static void define_c_function(
  Tables *t, String *name, String *code, String *result_type) {
  ...
  if (has_prefix(declaration, "static ")) {
    define_local_function_declaration(t, decl_line);
  } else {
    define_function_declaration(t, decl_line);
  }
  define_code(t, definition);
}

static Constant *make_body(
  String *mangled_name, long param_count, String *code) {
...
static Constant *make_unique(String *mangled_name) {
  ...
  c->param_count = 0;
  c->code = NULL;
  // key_of(unique_item) = "unique__" + name_of ; but name_of here is the
  // mangled name ("std__NAME"); key uses name_of(self).
...
static void define_type(
  Tables *t, String *name, String *parent, PairList *fields, int is_public) {
...
static void define_internal_method(
  Tables *t, String *object_name, String *method_id, String *function_name) {
  Variable *v = get_or_define_variable(t, object_name, FOT_UNKNOWN);
  plist_set(v->internal_methods, method_id, function_name);
}
static void define_internal_type_function(
  Tables *t, String *object_name, String *code) {
  Variable *v = get_or_define_variable(t, object_name, FOT_UNKNOWN);
  v->type_function = code;
}
static void define_method_value(Tables *t, String *object_name,
  String *attribute_name, int is_string, String *sval, Constant *cval,
  int is_attribute_value) {
  Variable *v = get_or_define_variable(t, object_name, FOT_UNKNOWN);
  Variable *attr =
    get_or_define_variable(t, attribute_name, FOT_UNKNOWN_POLYMORPHIC);
  String *method_id = concat2(string_from_c("var"), range_of(attr->id, 4, -1));
  String *attribute_id = concat2(string_from_c("-"), method_id);
  if (plist_find(v->attributes, method_id) ||
      plist_find(v->attributes, attribute_id)) {
...
static void define_object(
  Tables *t, String *name, String *typename, String *arguments, int is_public) {
  Variable *v = new_variable_prototype();
  v->type = FOT_OBJECT;
  v->parent = typename;
...
static void define_polymorphic_function(
  Tables *t, String *name, int with_setter) {
  Variable *v = new_variable_prototype();
...
static FragmentList *cut_into_fragments(const String *helpers) {
  ...
  frags->cap = 64;
  frags->count = 0;
  frags->items = xmalloc(sizeof(Fragment) * frags->cap);
  ...
  for (long idx = 1; idx <= n; idx++) {
    ...
    if (chr == '\n') {
      no++;
      if (idx == n || (char_at(helpers, idx + 1) > ' ' &&
                        char_at(helpers, idx + 1) != '}')) {
        long end_idx = idx;
        while (end_idx > 1 && char_at(helpers, end_idx - 1) == '\n') {
          end_idx--;
        }
        if (end_idx > start_idx) {
          if (frags->count == frags->cap) {
            frags->cap *= 2;
            frags->items = realloc(frags->items, sizeof(Fragment) * frags->cap);
          }
          frags->items[frags->count].line_no = start_no;
          frags->items[frags->count].text =
            range_of(helpers, start_idx, end_idx);
          frags->count++;
        }
        start_no = no;
        start_idx = idx + 1;
...
static PairList *parse_fields(const String *fragment_text) {
  ...
  for (int li = 0; li < lines->count; li++) {
    ...
    while (i - 1 >= 1 && is_identifier_char(char_at(line, i - 1))) {
      i--;
    }
...
static String *extract_code(
  const String *fragment_text, long *param_count_out) {
...
static void collector_copy_fields(
  Builder *b, const char *kind, PairList *fields, const char *indent) {
  for (int i = 0; i < fields->count; i++) {
    ...
    if (has_suffix(fname, "[]")) {
      if (strcmp(ftype->data, "NODE *") != 0) {
        ...
        String *src;
        if (strcmp(base->data, "size") == 0 ||
            strcmp(base->data, "length") == 0) {
          src = string_from_c("size");
          (void)src_prefix;
        } else {
          Builder *sb = builder_new();
          builder_append_c(sb, kind);
          builder_append_c(sb, "->");
          builder_append_str(sb, base);
          src = builder_to_string(sb);
        }
        builder_append_c(b, indent);
        builder_append_c(b, "memcpy(new_");
        builder_append_c(b, kind);
        builder_append_c(b, "->");
        builder_append_str(b, base);
        builder_append_c(b, ", ");
        builder_append_str(b, src);
        builder_append_c(b, ", size*sizeof(");
        builder_append_str(b, ftype);
        builder_append_c(b, "));\n");
      }
    } else {
      int is_ptr = char_at(ftype, -1) == '*' &&
                   strcmp(ftype->data, "void *") != 0 &&
                   !has_prefix(ftype, "const ");
      if (is_ptr) {
        /* pass */
      } else if (strcmp(fname->data, "size") == 0 ||
                 strcmp(fname->data, "length") == 0) {
        builder_append_c(b, indent);
        builder_append_c(b, "new_");
        builder_append_c(b, kind);
        builder_append_c(b, "->");
        builder_append_str(b, fname);
        builder_append_c(b, " = size;\n");
      } else {
        builder_append_c(b, indent);
        builder_append_c(b, "new_");
        builder_append_c(b, kind);
...
static void collector_collect_fields(
  Builder *b, const char *kind, PairList *fields) {
  for (int i = 0; i < fields->count; i++) {
    String *fname = fields->items[i].key;
    String *ftype = fields->items[i].value;
    if (has_suffix(fname, "[]")) {
      if (strcmp(ftype->data, "NODE *") == 0) {
        ...
        builder_append_c(b, "->");
        builder_append_str(b, base);
        builder_append_c(b, "[i] = collect_node(");
        builder_append_c(b, kind);
        builder_append_c(b, "->");
        builder_append_str(b, base);
        builder_append_c(b, "[i]);\n");
        builder_append_c(b, "    }\n");
      }
    } else {
      int is_ptr =
        char_at(ftype, -1) == '*' && strcmp(ftype->data, "void *") != 0 &&
      ...
      if (is_ptr) {
        ...
        builder_append_c(b, "->");
        builder_append_str(b, fname);
        builder_append_c(b, " = collect_");
        builder_append_str(b, coll);
        builder_append_c(b, "(");
        builder_append_c(b, kind);
        builder_append_c(b, "->");
        builder_append_str(b, fname);
...
static String *create_collector(
  const char *kind, const String *type, PairList *fields) {
  int contains_size = 0, contains_length = 0;
  int last_array = -1;
  for (int i = 0; i < fields->count; i++) {
    if (strcmp(fields->items[i].key->data, "size") == 0) {
      contains_size = 1;
    }
    if (strcmp(fields->items[i].key->data, "length") == 0) {
      contains_length = 1;
    }
    if (has_suffix(fields->items[i].key, "[]")) {
      last_array = i;
    }
  }
  String *item_type =
    (last_array >= 0) ? fields->items[last_array].value : NULL;
  String *array_size = NULL;
  if (contains_length) {
    array_size = string_from_c("length");
  } else if (contains_size) {
    array_size = string_from_c("size");
  } else if (item_type) {
    String *singular =
      without_suffix_c(range_of(fields->items[last_array].key, 1, -3), "s");
  ...
  if (array_size) {
    builder_append_c(b, "    long size = ");
    builder_append_c(b, kind);
    builder_append_c(b, "->");
    builder_append_str(b, array_size);
    builder_append_c(b, "; // might be garbage if already collected\n");
  }
  if (is_node) {
    builder_append_c(b, "    new_node = allocate(sizeof(");
    builder_append_str(b, (String *)type);
    builder_append_c(b, "));\n");
    builder_append_c(b, "    new_node->type = node->type;\n");
    builder_append_c(b, "    *(void **)node = ENCODE_ADDRESS(new_node);\n");
    builder_append_c(
      b, "    new_node->attributes = collect_attributes(node->attributes);\n");
    collector_copy_fields(b, kind, fields, "    ");
  } else {
    builder_append_c(b, "    int is_collected = IS_COLLECTED(");
    builder_append_c(b, kind);
    builder_append_c(b, ");\n");
    builder_append_c(b, "    if (is_collected) {\n");
    builder_append_c(b, "      void *new_location = *(void **)");
    builder_append_c(b, kind);
    builder_append_c(b, ";\n");
    ...
    if (array_size) {
      if (item_type) {
        builder_append_c(b, "      new_");
        builder_append_c(b, kind);
        builder_append_c(b, " = allocate(ALLOCATION_SIZE(sizeof(");
        builder_append_str(b, (String *)type);
        builder_append_c(b, ")+size*sizeof(");
        builder_append_str(b, item_type);
        builder_append_c(b, ")));\n");
      }
      ...
      builder_append_c(b, "      new_");
      builder_append_c(b, kind);
      builder_append_c(b, " = allocate(sizeof(");
      builder_append_str(b, (String *)type);
      builder_append_c(b, "));\n");
    }
    collector_copy_fields(b, kind, fields, "      ");
    if (contains_size || contains_length) {
      ...
      builder_append_c(b, "      if (!IS_STATIC(");
      builder_append_c(b, kind);
      builder_append_c(b, ")) {\n");
      builder_append_c(b, "        if (IS_ALREADY_MARKED(");
      builder_append_c(b, kind);
      builder_append_c(b, ")) return ");
      builder_append_c(b, kind);
      builder_append_c(b, ";\n");
      builder_append_c(b, "        MARK(");
      builder_append_c(b, kind);
      builder_append_c(b, ")\n");
      builder_append_c(b, "      }\n");
      builder_append_c(b, "    }\n");
  ...
  if (!is_node) {
    builder_append_c(b, "    if (is_collected) *(void **)");
    builder_append_c(b, kind);
    builder_append_c(b, " = ENCODE_TO_LENGTH(new_");
    builder_append_c(b, kind);
    builder_append_c(b, ");\n");
  }
  builder_append_c(b, "    return new_");
  builder_append_c(b, kind);
  builder_append_c(b, ";\n  }");
  return builder_to_string(b);
}
...
static Needle WS(void) {
  return need_set(" \t\n\r");
}
static Needle ALT_SP_STAR(void) {
  return need_set(" *");
}
static Needle ALT_WS_STAR(void) {
  return need_set(" \t\n\r*");
}

// mangle that tolerates a NULL (undefined) argument
static String *mangle_opt(String *name) {
  return name ? mangle(name) : NULL;
}
...
#define KW_WS(word)                                                            \
  (has_prefix(head, word) && (is_ws(char_at(head, (long)strlen(word) + 1)) ||  \
                               char_at(head, (long)strlen(word) + 1) == 0))
  ...
  if (KW_WS("TYPE")) {
    ...
    String *parent_name =
      contains_c(h, "->") ? trim(behind_str(h, "->")) : NULL;
    PairList *fields = parse_fields(text);
    define_type(
      t, mangle(typename), mangle_opt(parent_name), fields, is_public);
    if (parent_name == NULL || fields->count > 0) {
      String *type;
      if (parent_name == NULL && fields->count == 0) {
        type = string_from_c("SIMPLE_NODE");
      } else {
        type = to_upper_case(truncate_until_str(typename, "::"));
      }
      ...
      define_internal_method(
        t, mangle(typename), string_from_c("_collect"), fname);
    }
    return;
  }
  if (KW_WS("RECORD")) {
    ...
    const char *kind =
      to_lower_case(behind_needle(name, need_char('_'), -1))->data;
  ...
  if (KW_WS("UNION")) {
    ...
    for (int i = 0; i < t->unions->count; i++) {
      if (strcmp(t->unions->items[i].name->data, name->data) == 0) {
        ue = &t->unions->items[i];
        break;
      }
    }
    if (!ue) {
      if (t->unions->count == t->unions->cap) {
        t->unions->cap *= 2;
        t->unions->items =
          realloc(t->unions->items, sizeof(UnionEntry) * t->unions->cap);
      }
    ...
    if (ue->variants->count == ue->variants->cap) {
      ue->variants->cap *= 2;
      ue->variants->items =
        realloc(ue->variants->items, sizeof(UnionVariant) * ue->variants->cap);
    }
  ...
  if (KW_WS("ATTRIBUTE")) {
    ...
    String *qualified;
    if (contains_c(attribute_name, "::")) {
      qualified = mangle(attribute_name);
    } else {
      qualified = concat2(string_from_c("std__"), attribute_name);
    }
    (void)qualified;
    // define_attribute -> define_method value = "-var_std_types__undefined"
    // (string)
  ...
  if (KW_WS("METHOD")) {
    ...
    String *result_type = (!is_empty(type))
                            ? concat2(string_from_c("static "), type)
                            : string_from_c("static void");
    String *name = truncate_until_needle(h, ALT_SP_STAR(), -1);
    String *object_name =
      truncate_until_needle(before_char(h, '/'), ALT_SP_STAR(), -1);
    ...
    if (is_empty(function_name)) {
      code = extract_code(text, &parameter_count);
    }
    ...
    if (char_at(method_name, 1) == '_') {
      if (strcmp(method_name->data, "_type") == 0) {
        ...
        builder_append_str(bb, mangled);
        builder_append_c(bb, "___type, \"");
        builder_append_str(bb, name);
        builder_append_c(bb, "\"");
        bfn_add(ctx, bb->data);
        ...
        if (is_empty(function_name)) {
          function_name =
            concat2(concat2(mangle(object_name), string_from_c("__")),
              mangle(method_name));
        }
        if (code) {
          define_c_function(t, function_name, code, result_type);
        }
        define_internal_method(
          t, mangle(object_name), mangle(method_name), function_name);
      }
    } else {
      String *qualified;
      if (contains_c(method_name, "::")) {
        qualified = mangle(method_name);
      } else {
        qualified = concat2(string_from_c("std__"), method_name);
      }
      String *mangled_name =
        concat2(concat2(mangle(object_name), string_from_c("__")), qualified);
      Builder *bb = builder_new();
      builder_append_c(bb, "entry__");
      builder_append_str(bb, mangled_name);
      builder_append_c(bb, ", \"");
      builder_append_str(bb, name);
      builder_append_c(bb, "\"");
      bfn_add(ctx, bb->data);
  ...
  if (KW_WS("OBJECT")) {
    ...
    define_object(
      t, mangle(object_name), mangle(typename), arguments, is_public);
    return;
  }
  if (KW("UNIQUE")) {
    String *h = behind_needle(head, WS(), 1);
  ...
  if (KW_WS("DATA")) {
    ...
    String *data_type =
      trim(between_needles(declaration, need_char('}'), need_char(';'), 1));
    String *field_lines =
      trim(between_needles(declaration, need_char('{'), need_char('}'), 1));
    PairList *fields = plist_new();
    ...
    const char *kind =
      to_lower_case(behind_needle(data_type, need_char('_'), -1))->data;
    String *code = create_collector(kind, data_type, fields);
    // prepend the function signature line
    Builder *sig = builder_new();
    builder_append_c(sig, "static ");
    builder_append_str(sig, data_type);
    builder_append_c(sig, " *collect_");
    builder_append_str(sig, to_lower_case(data_type));
...
static void build_internal_methods(Tables *t, const String *common_h) {
  String *buf = between_needles(
    common_h, need_string("typedef struct VTABLE {\n"), need_char('}'), 1);
  ...
  for (int li = 0; li < lines->count; li++) {
    ...
    builder_append_c(fim, "  FIM_");
    builder_append_str(fim, to_upper_case(name));
    builder_append_c(fim, ",\n");
    builder_append_c(names, "  \"");
    builder_append_str(names, name);
    builder_append_c(names, "\",\n");
    if (is_a_function) {
      String *parameters =
        between_needles(line, need_char('('), need_char(')'), -1);
      Builder *args = builder_new();
      // split parameters on ',' and take the identifier after the last
      // space/star
      long pstart = 0;
      for (long k = 0; k <= parameters->length; k++) {
        if (k == parameters->length || parameters->data[k] == ',') {
          String *param =
            range_of(parameters, pstart + 1, k); // 1-based inclusive
          String *argname = behind_needle(param, ALT_SP_STAR(), -1);
          if (args->length) {
            builder_append_c(args, ", ");
          }
      ...
      builder_append_str(code, name);
      builder_append_c(code, "(");
      builder_append_str(code, builder_to_string(args));
      builder_append_c(code, ");\n  }");
...
static const char *fot_name(FieldObjectType t) {
  ...
  case FOT_UNKNOWN_POLYMORPHIC:
    return "FOT_UNKNOWN_POLYMORPHIC";
  case FOT_TYPE:
    return "FOT_TYPE";
  case FOT_OBJECT:
    return "FOT_OBJECT";
  case FOT_POLYMORPHIC:
    return "FOT_POLYMORPHIC";
  case FOT_INITIALIZED:
    return "FOT_INITIALIZED";
  case FOT_DERIVED:
    return "FOT_DERIVED";
  case FOT_UNINITIALIZED:
    return "FOT_UNINITIALIZED";
  default:
    return "FOT_UNKNOWN";
  }
}
...
static String *constant_size(Constant *c) {
  if (c->kind == CONST_BODY) {
    char buf[32];
    snprintf(buf, sizeof(buf), "%ld", c->param_count);
...
static String *constant_value(Constant *c) {
  Builder *b = builder_new();
  if (c->kind == CONST_BODY) {
    builder_append_c(b, "{.func = entry__");
    builder_append_str(b, c->name);
    builder_append_c(b, "}");
  } else {
    static const RRule r[1] = {{{{0, "__"}}, 1, "::"}};
    String *disp = replace_all(c->uname, r, 1);
    builder_append_c(b, "{.str_8 = \"");
    builder_append_str(b, disp);
    builder_append_c(b, "\"}");
  }
  return builder_to_string(b);
}
  ...
  if (v->parent) {
    ...
    if (pv) {
      ...
      for (int i = 0; i < pf->count; i++) {
        plist_push(out, pf->items[i].key, pf->items[i].value);
      }
    }
  }
  for (int i = 0; i < v->fields->count; i++) {
    plist_push(out, v->fields->items[i].key, v->fields->items[i].value);
  }
  return out;
}
...
static void create_header_and_source(Tables *t, const char *module_name,
  ParseContext *ctx, String **header_out, String **enums_out,
  String **source_out) {
  ...
  if (t->defined_namespaces->count) {
    ...
    for (int i = 0; i < t->defined_namespaces->count; i++) {
      builder_append_c(defined_namespaces_table, "  {\"");
      builder_append_str(
        defined_namespaces_table, t->defined_namespaces->items[i]);
      builder_append_c(defined_namespaces_table, "\", 1, 0},\n");
    }
    // without_suffix ",\n"
    defined_namespaces_table->length -= 2;
    defined_namespaces_table->data[defined_namespaces_table->length] = '\0';
    builder_append_c(defined_namespaces_table, "\n};\n");
  }
  ...
  for (int i = 0; i < t->local_function_declarations->count; i++) {
    builder_append_str(
      local_function_declarations, t->local_function_declarations->items[i]);
  }
  for (int i = 0; i < t->function_declarations->count; i++) {
    builder_append_str(
      function_declarations, t->function_declarations->items[i]);
  }
  ...
  if (t->constants->count) {
    ...
    for (int i = 0; i < t->constants->count; i++) {
      ...
      if (is_first) {
        builder_append_c(constants_enumeration, " = 1");
      }
      builder_append_c(constants_enumeration, ",\n");
      is_first = 0;
      if (c->kind == CONST_BODY) {
    ...
    constants_enumeration->length -= 2;
    constants_enumeration->data[constants_enumeration->length] = '\0';
    builder_append_c(constants_enumeration, "\n};\n");
    constants_table->length -= 2;
    constants_table->data[constants_table->length] = '\0';
    builder_append_c(constants_table, "\n};\n");
  }
  ...
  if (t->variables->count) {
    ...
    for (int vi = 0; vi < t->variables->count; vi++) {
      ...
      int fieldless = (all_fields->count == 0 &&
                       strcmp(fullname->data, "tabular_function") != 0);
      String *struct_type =
        fieldless ? string_from_c("SIMPLE_NODE") : struct_name;
      ...
      if (!fieldless) {
        ...
        for (int fi = 0; fi < all_fields->count; fi++) {
          ...
          builder_append_c(params, "    ");
          builder_append_str(params, type_and_name(all_fields->items[fi].value,
                                       all_fields->items[fi].key));
        }
        ...
        for (int fi = 0; fi < all_fields->count; fi++) {
          builder_append_c(struct_declarations, "  ");
          builder_append_str(
            struct_declarations, type_and_name(all_fields->items[fi].value,
                                   all_fields->items[fi].key));
          builder_append_c(struct_declarations, ";\n");
        }
        builder_append_c(struct_declarations, "} ");
    ...
    for (int vi = 0; vi < t->variables->count; vi++) {
      ...
      String *struct_name =
        typename ? to_upper_case(truncate_until_str(typename, "__"))
                 : string_from_c("SIMPLE_NODE");
      if (contains_char(arguments, '{')) {
        arguments =
          between_needles(arguments, need_char('{'), need_char('}'), 1);
      }
    ...
    for (int vi = 0; vi < t->variables->count; vi++) {
      ...
      if (do_print_var_no) {
        char buf[32];
        snprintf(buf, sizeof(buf), " = FIRST_VAR+%ld", var_no);
      ...
      if (!(internal_methods->count == 0 && fields->count == 0)) {
        ...
        for (int mi = 0; mi < internal_methods->count; mi++) {
          ...
          builder_append_str(
            attributes_table, to_upper_case(internal_methods->items[mi].key));
          builder_append_c(attributes_table, ", {");
          builder_append_str(
            attributes_table, internal_methods->items[mi].value);
          builder_append_c(attributes_table, "}},\n");
        }
        attributes_table->length -= 2;
        attributes_table->data[attributes_table->length] = '\0';
        builder_append_c(attributes_table, "\n};\n");
      }
    ...
    variables_enumeration->length -= 2;
    variables_enumeration->data[variables_enumeration->length] = '\0';
    builder_append_c(variables_enumeration, "\n};\n");
    ...
    for (int vi = 0; vi < t->variables->count; vi++) {
      ...
      String *name =
        is_local ? v->fullname : truncate_until_str(v->fullname, "__");
      ...
      if (vt == FOT_TYPE || vt == FOT_OBJECT || vt == FOT_DERIVED) {
        ...
        if (parent) {
          ...
          builder_append_c(pb, "\"");
          builder_append_str(pb, pname);
          builder_append_c(pb, "\\000");
          builder_append_str(pb, demangle_ns(pnamespace));
          builder_append_c(pb, "\"");
          parent_name = builder_to_string(pb);
        } else {
          parent_name = string_from_c("NULL");
        }
        builder_append_c(variables_table, "    {");
      ...
      if (vt == FOT_TYPE) {
        long methods_count =
          (fields->count == 0 ? 0 : 1) + internal_methods->count;
        char buf[48];
        snprintf(
          buf, sizeof(buf), "    {.methods_count = %ld}, 0,\n", methods_count);
        builder_append_c(variables_table, buf);
      ...
      if (vt == FOT_TYPE) {
        ...
        if (internal_methods->count == 0 && fields->count == 0) {
          ...
          builder_append_c(variables_table, "__internal_methods");
        }
        builder_append_c(variables_table, ",\n");
      } else if (vt == FOT_OBJECT) {
        builder_append_c(variables_table, "    NULL,\n");
    ...
    variables_table->length -= 2;
    variables_table->data[variables_table->length] = '\0';
    builder_append_c(variables_table, "\n};\n");
  }

  // --- module table --------------------------------------------------------
  const char *defined_ns_tab =
    t->defined_namespaces->count ? "defined_namespaces" : "NULL";
  ...
  const char *var_tab = t->variables->count ? "variables_table" : "NULL";
  {
    ...
    snprintf(buf, sizeof(buf),
      ...
      "%d,\n  %d, %ld,\n  %s,\n  %s, %s,\n  %s, %s\n};\n",
      module_name, module_name, module_name, 0, t->defined_namespaces->count, 0,
      t->constants->count, var_no, "NULL", defined_ns_tab, "NULL", const_tab,
      var_tab);
...
int main(int argc, char **argv) {
  ...
  while ((de = readdir(d))) {
    ...
    if (l >= 9 && strcmp(de->d_name + l - 9, ".template") == 0) {
      if (ncount == ncap) {
        ncap = ncap ? ncap * 2 : 32;
        names = realloc(names, sizeof(char *) * ncap);
      }
  ...
  // 3) build the TAG enum + union struct declarations
  {
    ...
    for (int ui = 0; ui < t->unions->count; ui++) {
      ...
      for (int vi = 0; vi < vl->count; vi++) {
        ...
        for (int k = 0; k < tags->count; k++) {
          if (strcmp(tags->items[k]->data, vl->items[vi].variant_name->data) ==
              0) {
            seen = 1;
            break;
          }
        ...
        if (!seen) {
          slist_push(tags, vl->items[vi].variant_name);
        }
      }
    }
    ...
    for (int k = 0; k < tags->count; k++) {
      builder_append_c(td, "  ");
      builder_append_str(td, tags->items[k]);
      builder_append_c(td, ",\n");
    }
    td->length -= 2;
    td->data[td->length] = '\0'; // range 1 -3 (drop trailing ",\n")
    builder_append_c(td, "\n} TAG;\n");
  ...
  for (int ui = 0; ui < t->unions->count; ui++) {
    ...
    for (int vi = 0; vi < ue->variants->count; vi++) {
      ...
      for (int fi = 0; fi < uv->fields->count; fi++) {
        builder_append_c(ud, "  ");
        builder_append_str(ud, type_and_name(uv->fields->items[fi].value,
                                 uv->fields->items[fi].key));
        builder_append_c(ud, ";\n");
      }
      builder_append_c(ud, "} ");
  ...
  for (int i = 0; i < t->include_files->count; i++) {
    builder_append_c(inc, "#include <");
    builder_append_str(inc, t->include_files->items[i]);
    builder_append_c(inc, ">\n");
  }
  String *includes = builder_to_string(inc);

  // builtin_function_names buffer, trailing ",\n" removed
  String *bfn =
    without_suffix_c(builder_to_string(ctx.builtin_function_names), ",\n");

  const char *HEADER = "// This file is automatically generated by "
                       "\"builtins.sim\"; do not edit manually!\n";
  ...
  builder_append_c(H, "\n");
  {
    char b[96];
    snprintf(b, sizeof(b),
      "extern BUILTIN_FUNCTION_NAME builtin_function_names[%ld];\n",
      ctx.builtin_function_count);
    builder_append_c(H, b);
  }
  ...
  builder_append_c(
    H, "extern BUILTIN_FUNCTION_NAME builtin_function_names[];\n");
  ...
  builder_append_c(C, "\n");
  builder_append_c(C, "#define _GNU_SOURCE\n"
                      "#include <stdint.h>\n#include <stdlib.h>\n#include "
                      "<stdio.h>\n#include <string.h>\n"
                      "#include <errno.h>\n#include <inttypes.h>\n#include "
                      "<limits.h>\n#include <unistd.h>\n"
  ...
  builder_append_c(C, "\n");
  {
    char b[96];
    snprintf(b, sizeof(b),
      "BUILTIN_FUNCTION_NAME builtin_function_names[%ld] = {\n",
      ctx.builtin_function_count);
    builder_append_c(C, b);
  }
  ...
  fwrite(out_h->data, 1, (size_t)out_h->length, fh);
  fclose(fh);
  FILE *fc = fopen(out_c_path, "wb");
  fwrite(out_c->data, 1, (size_t)out_c->length, fc);
  fclose(fc);
  fprintf(stderr, "wrote %s (%ld bytes) and %s (%ld bytes)\n", out_h_path,
    out_h->length, out_c_path, out_c->length);
  return 0;
}
